...
async function modifyRouter(name) {
  const cwd = process.cwd();
  const filePath = path.resolve(cwd, 'app', 'router.js');
  const content = fs.readFileSync(filePath, 'utf-8');
  const index = content.indexOf('module.exports = app => {') + 25;
  const arr = content.split('');
  arr.splice(index, 0, `\r  require('./router/${name}')(app);\r`);
